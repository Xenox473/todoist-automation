...
class SyncReclaim():
    ...
    def create_reclaim_task(self, todoist_task, reclaim_task = None):
        message = "Updated" if reclaim_task is not None else "Created"
        ...
        reclaim_task.description = todoist_task.url
        ...
        if todoist_task.due is not None:
            ...
            if "This_Week" in todoist_task.labels:
                reclaim_task.start_date = self.today
                reclaim_task.due_date = datetime.now() + timedelta(days = 6 - self.today.weekday())
            else:
        ...
        reclaim_task.save()
        print(message + " Reclaim task: " + todoist_task.content + " " + reclaim_task.due_date.strftime("%Y-%m-%d"))
        return True
    
    def grab_todoist_tasks(self):
        # Grab all tasks that are labelled "This_Week"
        tasks = list(filter(lambda x: x.labels is not None and "This_Week" in x.labels, self.todoist_api.get_tasks()))
        return tasks
    ...
    def cleanup_reclaim_tasks(self, reclaim_tasks, todoist_tasks):
        ...
        for task in reclaim_tasks:
            if task.description not in map(lambda x: x.url, todoist_tasks):
                # Mark tasks as completed in Reclaim if they are completed in Todoist
                try:
                    todoist_task = self.todoist_api.get_task(task.description.split("=")[-1])
    ...
    def sync(self):
        ...
        for task in todoist_tasks:
            if task.url not in map(lambda x: x.description, reclaim_tasks):
                self.create_reclaim_task(task)
            else:
                reclaim_task = list(filter(lambda x: x.description == task.url, reclaim_tasks))[0]
